...
# Q 2.2.2
# implement softmax
probs = forward(h1,params,'output',softmax) # [40 * 4]
        ...
        for row in range(len(v)):
            if len(v.shape) > 1:
                for col in range(len(v[0])):
                    params[k][row][col] += eps
                    h_1_eps1 = forward(xb, params, 'layer1')
                    pred_y_eps1 = forward(h_1_eps1, params, 'output',softmax)
                    loss1, _ = compute_loss_and_acc(yb, pred_y_eps1)
                    params[k][row][col] -= 2 * eps
                    h_1_eps2 = forward(xb, params, 'layer1')
                    pred_y_eps2 = forward(h_1_eps2, params, 'output',softmax)
                    loss2, _ = compute_loss_and_acc(yb, pred_y_eps2)
                    params['grad_' + k][row][col] = (loss1 - loss2) / 2 * eps
            else:
                params[k][row] += eps
                h_1_eps1 = forward(xb, params, 'layer1')
                pred_y_eps1 = forward(h_1_eps1, params, 'output',softmax)
                ...
                pred_y_eps2 = forward(h_1_eps2, params, 'output',softmax)
                loss2, _ = compute_loss_and_acc(yb, pred_y_eps2)
                params['grad_' + k][row] = (loss1 - loss2) / 2 * eps
